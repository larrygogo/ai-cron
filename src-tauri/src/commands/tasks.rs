...
#[tauri::command]
pub fn create_task(req: CreateTaskRequest, db: State<'_, DbConn>) -> Result<Task, String> {
    let conn = db.0.lock().map_err(|e| e.to_string())?;
...
pub fn update_task(
    ...
    // Bind parameters using rusqlite's params_from_iter workaround via raw binding
...
#[tauri::command]
pub fn delete_task(id: String, db: State<'_, DbConn>) -> Result<(), String> {
    let conn = db.0.lock().map_err(|e| e.to_string())?;
...
#[tauri::command]
pub fn set_task_enabled(id: String, enabled: bool, db: State<'_, DbConn>) -> Result<(), String> {
    let conn = db.0.lock().map_err(|e| e.to_string())?;
    let now = Utc::now().to_rfc3339();
    conn.execute(
